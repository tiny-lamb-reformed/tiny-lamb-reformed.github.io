...
def process_all():
    ...
    with open("source/_data/navigation.yml") as f:
...
def generate_category_sidebar():
    nav = {}

    categories = ["馬太福音查經", "聖樂欣賞", "書籍評論", "經文小亮光"]
    for category in categories:
        series = sorted(
            [a for a in articles if a["category"] == category], key=lambda a: a["path"]
        )
        for a in series:
            a["sidebar"] = {"nav": category}
            write_article(a)
        ...
        nav[category] = [
            {
                "title": category,
                "children": [
                    {"title": a["title"], "url": f'/posts/{a["id"]}/'} for a in series
                ],
            }
        ]

    with open("source/_data/navigation.yml", "r+") as f:
        while line := f.readline():
            if line == "### auto generated by offline_process.py ###\n":
                break
        f.seek(f.tell())
        f.write(
            "\n".join([str(f"{k}: {v}") for k, v in nav.items()])
            + "\n### end auto generation ###\n"
        )
        f.truncate()
